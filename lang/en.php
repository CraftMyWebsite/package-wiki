<?php

return [
	"title" => [
		"dashboard_title" => "Wiki",
		"add_category" => "Add category",
		"add_article" => "Add article",
		"edit_category" => "Edit category",
        "edit_article" => "Edit an article",
		"dashboard_desc" => "Manage the wiki of your website",
		"manage" => "Manage",
		"actif" => "Active :",
		"inactif" => "Inactive :",
		"inactifcat" => "Inactive categories",
		"inactifart" => "Inactive articles",
	],
	"button" => [
		"add_category" => "Add a category",
		"add_article" => "Add an article",
	],
	"articles" => [
		"undefined" => "Articles & category <br> undefined:",
		"articles" => "Articles",
		"categories" => "Category",
	],
	"add" => [
		"category_name" => "Name",
		"category_name_placeholder" => "Enter the name of the category",
		"category_description" => "Description",
		"category_description_placeholder" => "Enter the description",
		"category_icon" => "Icon",
		"category_icon_placeholder" => "Example: fas fa-users",
		"category_slug" => "Link",
		"category_slug_placeholder" => "Comme-with-us",
		"article_title" => "Title",
		"article_title_placeholder" => "Enter the title",
		"article_categorie" => "CCategory",
		"article_content" => "Content",
		"article_content_placeholder" => "Content of this article",
		"hint_icon" => "Get all this icons on the website",
	],
	"edit" => [
		"category_enable" => "Enable this category ?",
		"article_enable" => "Enable this article ?",
	],
];